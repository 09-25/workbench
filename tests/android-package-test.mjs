...
check('本次 Android 更新版本号正确', /versionCode\s+14/.test(gradle) && /versionName\s+"1\.0\.13"/.test(gradle));
check('APP_VERSION 常量与 APK 版本一致', new RegExp('APP_VERSION = "' + gradle.match(/versionName\s+"([^"]+)"/)[1] + '"').test(readFileSync(resolve(root, 'app.js'), 'utf8')));
